...
axis square
colorbar
print('ex3input3','-depsc')

figure(3)
% predim = interp2(X1,X2, predim, Zint1, Zint2);
g = imagesc(x1,x2,pred1);
% g = mesh(X1,X2, predim);
set(g,'alphadata',~isnan(pred1));
set(g,'alphadata',~isnan(trueim));
set(gca,'xtick',[],'xticklabel',[])
set(gca,'ytick',[],'yticklabel',[])
% caxis([0.95 1.05]);
...
axis square
colorbar
print('ex3pred3it100','-depsc')

figure(4)
% predim = interp2(X1,X2, predim, Zint1, Zint2);
g = imagesc(x1,x2,pred2);
% g = mesh(X1,X2, predim);
set(g,'alphadata',~isnan(pred2));
set(g,'alphadata',~isnan(trueim));
set(gca,'xtick',[],'xticklabel',[])
set(gca,'ytick',[],'yticklabel',[])
% caxis([0.95 1.05]);
...
axis square
colorbar
print('ex3pred3it200','-depsc')

figure(5)
% predim = interp2(X1,X2, predim, Zint1, Zint2);
g = imagesc(x1,x2,pred3);
% g = mesh(X1,X2, predim);
set(g,'alphadata',~isnan(pred3));
set(g,'alphadata',~isnan(trueim));
set(gca,'xtick',[],'xticklabel',[])
set(gca,'ytick',[],'yticklabel',[])
% caxis([0.95 1.05]);
...
axis square
colorbar
print('ex3pred3it800','-depsc')

figure(6)
p=plot(x2,trueim(15,:),'r','LineWidth',2.5);
hold on
q=plot(x2,inputim(15,:),':m','LineWidth',2);
r=plot(x2,pred1(15,:),':k','LineWidth',1.5);
s=plot(x2,pred2(15,:),'--k','LineWidth',1.5);
t=plot(x2,pred3(15,:),'-k','LineWidth',1.5);
legend([p q r s t],{'True','Calderon','Step 100','Step 200','Step 800'},'Location','south')
ax = gca;
ax.FontSize=15;
set(g,'alphadata',~isnan(trueim));
set(gca,'xtick',[],'xticklabel',[])
hold off
print('ex3cutoff3','-depsc')